// SUIT — diagnostic: dump the Soroban auth entries + recipient existence for a
// deposit vs a withdraw, to explain why Freighter rejects the withdraw envelope.
...
const POOL = "CAXFFBZHC7CFYFOQSMV57TAY2CEO6Y2GMOQKLKSERD4O4DBMLFSMDA63";
const START = 3236050;
...
function dumpAuth(label, tx) {
  const op = tx.operations[0];
  const auths = op.auth || [];
  console.log(`\n[${label}] operation auth entries: ${auths.length}`);
  auths.forEach((a, i) => {
    const cred = a.credentials();
    console.log(`  #${i} credentials switch = ${cred.switch().name}`);
  });
}

async function accountExists(g) {
  try { await server.getAccount(g); return true; } catch { return false; }
}

(async () => {
  console.log("SRC:", SRC);
  const recipient = "GC2Q2S47LNBPW7D466IINEOA4Q4R576X6VWM43XMX5TQCYO7ZH43XCJL"; // from the user's screenshot
  console.log("recipient", recipient, "exists on testnet?", await accountExists(recipient));

  // fresh deposit so we have an unspent note
  ...
  dumpAuth("DEPOSIT assembled", depAssembled);
  // submit deposit
  depAssembled.sign(kp);
  const sent = await server.sendTransaction(depAssembled);
  console.log("deposit submit:", sent.status, sent.hash || "");
  ...
  // now build the WITHDRAW and dump its auth
  leaves = await syncLeaves();
  const myIdx = leaves.findIndex(l => l === outCommit);
  const root = treeRoot(leaves);
  const W = D; // full withdrawal (change 0) — matches the user's MAX action
  const change = D - W;
  ...
  dumpAuth("WITHDRAW assembled (full, change 0)", wAssembled);

  // submit via keypair to confirm contract accepts it (isolate from Freighter)
  wAssembled.sign(kp);
  const wsent = await server.sendTransaction(wAssembled);
  console.log("\nwithdraw submit (keypair):", wsent.status, wsent.hash || JSON.stringify(wsent.errorResult));
  for (let i = 0; i < 60; i++) { await new Promise(r => setTimeout(r, 1500)); const g = await server.getTransaction(wsent.hash); if (g.status !== "NOT_FOUND") { console.log("withdraw final:", g.status); break; } }
  process.exit(0);
