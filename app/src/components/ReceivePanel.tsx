...
import { withdraw, getNotes, getActiveToken, getRelayerInfo, enableAndFundUSDC, getWalletTokenBalance, CONFIG, UTXONote, stroopsToXlm } from '../lib/suit';
...
export default function ReceivePanel() {
  ...
  async function handleFaucet() {
    ...
    try {
      ...
      setTokenBal(bal);
      setFaucet({ busy: false, msg: `1,000 test USDC minted. Balance: ${Number(bal).toLocaleString(undefined, { maximumFractionDigits: 2 })} USDC`, err: '' });
      refreshBalance();
    } catch (e: any) {
      ...
      {sym === 'USDC' && (
        <div style={{ marginBottom: 14, padding: '10px 14px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
            <span style={{ fontSize: 11.5, color: 'var(--text-3)' }}>Need test USDC?</span>
