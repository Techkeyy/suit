// SUIT Protocol — circom → Soroban hex converter
//
// Converts snarkjs Groth16 JSON outputs into the byte payloads expected by
// the SUIT Groth16 verifier contract (BLS12-381). The encoding here is the
// authoritative source of truth and is matched 1:1 by the contract's
// `from_bytes` parsers.
//
// Usage:
//   circom-to-soroban-hex vk     build/verification_key.json
//   circom-to-soroban-hex proof  build/proof.json
//   circom-to-soroban-hex public build/public_signals.json
//
// Layout:
//   vk     : alpha(G1) | beta(G2) | gamma(G2) | delta(G2)
//            | ic_len(u32 BE) | IC...(G1 each)
//   proof  : a(G1) | b(G2) | c(G1)
//   public : len(u32 BE) | signal...(32-byte BE each)
//
// G1Affine serializes to 96 bytes uncompressed, G2Affine to 192 bytes,
// via arkworks CanonicalSerialize — the exact format soroban-sdk's
// G1Affine::from_array / G2Affine::from_array decode.
